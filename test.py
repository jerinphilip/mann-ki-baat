...
for filepath in tqdm(glob.iglob('data/*/*.txt')):
    ...
    #if src_lang in ['bn','or','ur']:

    if src_lang in ['or']:
        content = []
        for line in src_content:
            _, segments = segmenter(line, lang=lang)
            content.extend(segments)

        src_io, src_tgt_io = prepare(content, src_lang, tgt_lang)
...
def interse(lst1, lst2): 
    return set(lst1).intersection(lst2) 

mkb = defaultdict(set)
pib =defaultdict(set)

for filepath in glob.iglob('mkb/*/mkb.*'):
    file = filepath.split('/')[2]
    lang = file.split('.')[1]
    src_content = open(filepath,'r')
    words = set()
...
# l=0
# words = set()
# for filepath in glob.iglob('mkb/*/*'):
#     file = filepath.split('/')[2]
#     lang = file.split('.')[1]
#     src_content = open(filepath,'r')
...
# words = list(words)
#         #mkb[lang] = len(words)
# print(count,l, len(words))

file = open('./pib-filt/en-mr/train.mr','r')
vocab =set()
for line in file:
